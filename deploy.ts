...
async function dockerCmd ({ cmd, image, id, version, previous }: { cmd: 'tag' | 'push', image: string, id: string, version: string, previous?: string }) {
  const latestTag = `${registry}/${image}:latest`
  const versionTag = `${registry}/${image}:${version}`
  if (previous) {
    const previousTag = `${registry}/${image}:${previous}`
    await exec({ command: tmpl[cmd].replace('{source}', latestTag).replace('{target}', previousTag) });
  }
  await exec({ command: tmpl[cmd].replace('{source}', id).replace('{target}', versionTag) });
  await exec({ command: tmpl[cmd].replace('{source}', versionTag).replace('{target}', latestTag) });
}
...
async function main({
  ...
  await exec({ command: `yarn workspace ${workspace} version --patch` });
...
const pIndex = process.argv.findIndex(
...
const prevIndex = process.argv.findIndex(
  (arg) => arg.startsWith("--previous")
);
if (pIndex !== -1 && wIndex !== -1 && sIndex !== -1 && iIndex !== -1) {
  const path = process.argv[pIndex].split("=")[1];
  const workspace = process.argv[wIndex].split("=")[1];
  const service = process.argv[sIndex].split("=")[1];
  const image = process.argv[iIndex].split("=")[1];
  const previous = prevIndex !== -1 ? process.argv[prevIndex].split("=")[1] : undefined;
  main({ path, workspace, service, image, previous });
}
